...
MongoClient.connect(url, {
  ...
  router.post('/hierarchy', (req, res) => {
    if(req.body && req.body.college_id ) {
      ...
      if(!college_id) return res.json({ error: true, mssg: 'invalid request' });
      dbo.collection(TABLE_COLLEGES).findOne({ _id: college_id }, (err, result) => {
        if(err) return res.json({ error: true, mssg: 'invalid college id' });
        if(result && result.data) {
          res.json({ error: false, data: result.data });
        } else {
          if(err) return res.json({ error: true, mssg: 'college not configured yet' });
        }
      });
    } else {
